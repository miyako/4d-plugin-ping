...
double DiffMs(double endMs, double startMs){
    ...
    // endMs < startMs means the underlying clock went backwards (e.g. the
    // VERSIONWIN timeGetTime() fallback wraps its 32-bit ms counter roughly
    // every 49.7 days of uptime). There is no way to recover the true
    // elapsed time from two raw wrapped samples, so report 0 rather than
    // the previous "+ DBL_MAX" hack, which could produce a ~1.7e308 value
    // and overflow the fixed-size buffer in DoubleToString below.
        ...
        if(dwRetVal){
            ...
            if(pEchoReply->Status == IP_SUCCESS){
                ...
                *ttl = pEchoReply->Options.Ttl;				
                ...
                // Previously fell through with err left at 0 ("success"),
                // so a non-success status (unreachable, TTL expired, etc.)
                // caused the caller to format uninitialized bytes/ttl as if
                // they were a real response. IP_SUCCESS == 0, so Status is
                // guaranteed non-zero here and is safe to use directly.
                err = (int)pEchoReply->Status;
            }			
        }else{
            err = GetLastError();
...
int SendReceiveEchoResponse(SOCKET sock, const struct sockaddr *addr, socklen_t socklen, int limit, int seqId, int *bytes, int *ttl){
    ...
    *ttl = 0;
    ...
    if(sock == INVALID_SOCKET){
        // Previously fell through and returned err = 0 ("success") without
        // ever writing *bytes/*ttl, so the caller would format whatever
        // garbage happened to be on its stack as a real response. A raw
        // ICMP socket can fail to open (e.g. insufficient privilege), so
        // this must be reported as a real error, not silent success.
        return -2;
    }
...
void HOST_Ping(sLONG_PTR *pResult, PackagePtr pParams)
{
    ...
    // Upper-bound clamps: 'count' and 'limit' come straight from an ordinary
    // 4D method call with no special privilege required. Previously only
    // floor-clamped, an unbounded count could tie the process up sending
    // billions of pings, and an unbounded limit could both cause enormous
    // single-request waits and, on the Windows path, overflow 'timeOut * 1000'
    // (a plain int) before it's passed to IcmpSendEcho as a DWORD timeout.
    const int kMaxPingCount = 100;
    const int kMaxPingTimeoutSeconds = 30;
    
    if(count < 1)
        count = 1;
    if(count > kMaxPingCount)
        count = kMaxPingCount;
    
    if(limit < 1)
        limit = 1;
    if(limit > kMaxPingTimeoutSeconds)
        limit = kMaxPingTimeoutSeconds;
    
    CUTF8String host;
    hostName.copyUTF8String(&host);
    
    struct addrinfo hints, *res, *res0 = NULL;
    bool res0Allocated = false;
    ...
    if((err = getaddrinfo((const char *)host.c_str(), NULL, &hints, &res0))){
        ...
        // NOTE: this loop intentionally walks every address getaddrinfo
        // returns (a host with multiple A records is pinged once per
        // address); a 'done' flag previously existed here but was never
        // set anywhere, so it did nothing -- removed as dead code rather
        // than silently changing behavior by making it functional.
        ...
        // Guarantee a return value is always set, even if something above
        // throws (e.g. std::bad_alloc from a CUTF8String/ARRAY_TEXT
        // allocation). Previously an exception here would propagate all
        // the way to PluginMain's catch(...), which never calls
        // setReturn(), leaving 4D waiting on a return value that would
        // never arrive -- a hang, not a crash. -999 is a plugin-local
        // sentinel distinct from the per-ping error codes used above.
        if(res0Allocated)
            freeaddrinfo(res0);
        returnValue.setIntValue(-999);
    }
